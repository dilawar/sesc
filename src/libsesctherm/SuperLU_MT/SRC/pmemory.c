...
#define XPAND_HINT(memtype, new_next, jcol, param, max) {\
fprintf(stderr, "Storage for %12s exceeded; Current column %d; Need at least %d(currently is %d);\n",\
	memtype, jcol, new_next,max); \
...
#pragma _CRI guard ULOCK
#endif	
	{
	    ...
	    if ( new_next > Glu->nzumax ) {
	        XPAND_HINT("U columns", new_next, jcol, 7, Glu->nzumax);
	    }
	    *prev_next = nextu;
	    Glu->nextu = new_next;
...
#pragma _CRI guard LLOCK
#endif	
	{
	  ...
	  if ( new_next > Glu->nzlmax ) {
	      XPAND_HINT("L subscripts", new_next, jcol, 8, Glu->nzlmax);
	  }
	  *prev_next = nextl;
	  Glu->nextl = new_next;
...
#pragma _CRI guard LULOCK
	...
	if ( new_next > Glu->nzlumax ) {
	    XPAND_HINT("L supernodes", new_next, jcol, 6, Glu->nzlumax);
	}
	Glu->nextlu = new_next;
    } /* end of critical region */
