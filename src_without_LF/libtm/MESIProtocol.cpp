...
void MESIProtocol::write(MemRequest *mreq)
{
  ...
  #ifdef TLS
	  //WRITE HIT send out request without demanding data
	  if (l) {
		    changeState(l, MESI_TRANS_WR);
		    doSendWritePropagate(mreq);
		    return;
	  }
  #else
	  // hit in shared state
...
//  GI(l, !l->isValid());
  ...
  #ifdef TLS
	  l = pCache->allocateLine(mreq->getPAddr(), mreq,doWriteCB::create(this, mreq));
  #else  
...
#ifdef TLS
	//Set Epoch of newly allocated line
	if (l && mreq->isDataReq())
		{
			I (mreq->getEpoch());
			if (mreq->getEpoch()->getClock() !=0)
...
#ifdef TLS
//Send read hit below (needs data is false)
void MESIProtocol::doSendReadPropagate(MemRequest *mreq)
{
  I(pCache->getLine(mreq->getPAddr()));
  SMPMemRequest *sreq = SMPMemRequest::create(mreq, pCache, false);

  pCache->sendBelowUp(sreq,mreq); 
}
//Send write hit below (needs data is false)
void MESIProtocol::doSendWritePropagate(MemRequest *mreq)
{
  I(pCache->getLine(mreq->getPAddr()));
  SMPMemRequest *sreq = SMPMemRequest::create(mreq, pCache, false);
...
void MESIProtocol::readMissAckHandler(SMPMemRequest *sreq) 
{
  ...
  Line *l = pCache->getLine(addr);
 #ifdef TLS
	if (!l)//Line has been squashed, just send up request
	{
  ...
  if(sreq->getState() == MESI_INVALID) { 
    if(l->getState() == MESI_TRANS_RD_MEM)//Change state in case of read miss not otherwise
    changeState(l, MESI_EXCLUSIVE);
  } else {
    if(l->getState() == MESI_TRANS_RD)
    changeState(l, MESI_SHARED);
    //changeState(l, MESI_EXCLUSIVE);
  }
  #ifdef TLS
	if (sreq->isDataReq()){
		//Only on a read miss check if others exposed flag is to be set
		if (sreq->needsData())
			if (sreq->getSetOthersExposed())
				l->getCacheFlags()->setOE();
		//(l->getCacheFlags())->setWordER(pCache->getOffsetInLine(addr));
	}
  #endif
...
void MESIProtocol::writeMissAckHandler(SMPMemRequest *sreq) 
{
  ...
  changeState(l, MESI_MODIFIED);
	...
	if (sreq->isDataReq()){
		if (sreq->getSetOthersExposed())
			l->getCacheFlags()->setOE();
		if (sreq->getClearOthersExposed())
			l->getCacheFlags()->clearOE();
		(l->getCacheFlags())->setWordWM(pCache->getOffsetInLine(addr));
	}
  #endif
  pCache->writeLine(addr);
  pCache->concludeAccess(sreq->getOriginalRequest());
